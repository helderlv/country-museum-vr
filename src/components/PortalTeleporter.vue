...
const props = defineProps({
  ...
  material: { type: String, default: 'shader: standard; color: #222244; side: double' },
...
<template>
  <a-entity :id="`portal-${id}`" geometry="primitive: circle; radius: 1" :material="material" clickable
    :teleport-camera-rig="`
      x: ${x};
      y: ${y};
      z: ${z};
    ...
    <a-text v-if="label" align="center" :value="label" position="0 1.2 0"></a-text>

    <a-entity v-if="cameraEffect" :id="`portal-look-at-${id}`" :look-at="`target: ${cameraSelector};`"></a-entity>

    <Teleport to="a-scene" v-if="cameraEffect">
      <a-entity :rotation="`0 ${cameraRot} 0`">
        <a-entity :id="`camera-${id}`" :secondary-camera="`
          ...
          :bind-rotation="`target: #portal-look-at-${id};`"></a-entity>
      </a-entity>
    </Teleport>
